...
x_n = 0.1 # posicao x atual d orobo
...
def example ():
    ...
    callback_pose ) # declaracao do topico onde sera lido o estado

    # Inicializa os nos do robo para enviar os marcadores para o rviz
    ...
    # Define uma variavel que controlar a frequencia de execucao deste no 
    rate = rospy.Rate(freq)
    sleep (0.2)
    # O programa no consiste no codigo dentro deste while
    while not rospy. is_shutdown (): # "Enquanto o programa nao ser assassinado"
        # Incrementa o tempo
        i = i + 1
        time = i / float (freq)
        # Obtem a trajetoria exemplo de referencia "constante neste exemplo"
        ...
        # Espera por um tempo desejada de forma a manter a frequencia
        rate.sleep ()
