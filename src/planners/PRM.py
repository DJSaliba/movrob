...
class PRM(Astar):
    def __init__(self,image_path, scale = 0.125,neighborhood=4, n=60):
    ...
    def save_graph_img(self):
        ...
        for k,v in tqdm(self.graph.items()):
            ...
            for n in v.neighbors:
                plt.plot([k[0],n[0]],[k[1],n[1]],'ob', linestyle="-",lw=0.1)
                plt.plot(k[0], k[1], 'or')
                plt.plot(n[0], n[1], 'or')
        for k,n in tqdm(zip(self.path[:-1],self.path[1:])):
            print(k,n)
            plt.plot([k[0],n[0]],[k[1],n[1]],'og', linestyle="-",lw=0.5,markersize=1)
